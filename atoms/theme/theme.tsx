...
window.addEventListener('resize', throttle(updateMediaQueries, 30))
class ThemeContainer extends React.Component<any, ThemeInterface> {
  constructor(props: any) {
    super(props)
    this.state = sharedState
    this.syncToSharedState = throttle(this.syncToSharedState, 30)
  }
  id = uuid()
  isDestroyed = false
  componentDidMount() {
  ...
  componentWillUnmount() {
    window.removeEventListener('resize', this.syncToSharedState)
    this.isDestroyed = true // we have a throttled listener that updates state, so we need this!
  }
  watchScreenSize() {
    window.addEventListener('resize', this.syncToSharedState)
  }
  syncToSharedState = () => {
